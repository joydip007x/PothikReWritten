...
public class CreateATour extends AppCompatActivity implements CancelCreateATour.CancelCreateATourLis {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Postit.setOnClickListener(v -> {
            ...
            String dateio=tourtime.getText().toString();
            ...
            if(tourtime.getText().toString().isEmpty() || dateio.length()!=10 ) {
                tourtime.setError("Correct format- DD/MM/YYYY ");
                tourtime.requestFocus();
                return;
            }
            if(!checkCorrectDate(dateio)){

                tourtime.setError("Give a correct &  practical date ");
                Toasty.info(getApplicationContext(),"Date format - DD/MM/YYYY ",Toasty.LENGTH_LONG,true).show();
                tourtime.requestFocus();
                return;
    ...
    private boolean checkCorrectDate(String dateio) {
       ...
       if(DateAndTime.backdated(dateio)) {

           System.out.println(" HEHE "+dateio);
           return  false;
       ...
       if( iy>=ay  ){
             ...
             if( im>= 1 &&  im<=12 ){
                 ...
                 if(id>=1  && id<=31){
                    ...
                    if(( im==4 || im==6 || im==11 || im==9) && id>=31) return  false;
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.exists()) {
                   ...
                   /// System.out.println("OOOOOOO"+dataSnapshot.child("agencyName").getValue().toString());
                    ...
                    baseinfo.put("Agency", requireNonNull(dataSnapshot.child("agencyName").getValue()).toString());
                    baseinfo.put("Phone", requireNonNull(dataSnapshot.child("number").getValue()).toString());
                    baseinfo.put("Email", requireNonNull(dataSnapshot.child("email").getValue()).toString());
